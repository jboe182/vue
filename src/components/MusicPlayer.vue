  ...
  <div v-if="audioStore.currentTrack" 
    ...
    <div v-if="!isExpanded" 
      ...
      <div class="flex items-center space-x-4">
        <div class="w-12 h-12 bg-gray-200 rounded overflow-hidden">
          ...
          />
          <div v-else class="w-full h-full bg-red-600 flex items-center justify-center text-white text-xs">
            ♪
          </div>
        </div>
        <div>
      ...
      <div class="flex items-center space-x-4" @click.stop>
        <button class="p-2 hover:bg-gray-100 rounded-full text-gray-400 hover:text-red-600 transition">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
          </svg>
        </button>
        <button @click="togglePlay" class="p-3 bg-gray-300 text-gray-700 rounded-full hover:bg-red-600 hover:text-white transition">
        ...
        <button @click.stop="toggleExpand" class="p-2 hover:bg-gray-100 rounded-full ml-2 text-gray-400 hover:text-red-600 transition">
           <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <div v-else class="flex-1 flex flex-col bg-white overflow-y-auto">
      <div class="p-4 flex justify-end">
        <button @click="toggleExpand" class="p-2 hover:bg-gray-100 rounded-full text-gray-400 hover:text-red-600 transition">
      ...
      <div class="flex-1 flex flex-col md:flex-row p-4 md:p-8 gap-8">
        ...
        <div class="md:hidden flex flex-col items-center w-full">
          ...
          <div 
            ...
            :class="[
              showLyrics 
                ...
                : 'w-64 h-64 rounded-lg shadow-xl'
            ]"
          ...
          >
            <img 
              ...
              class="w-full h-full object-cover"
            />
            <div v-else class="w-full h-full bg-gradient-to-br from-red-600 to-gray-800 flex items-center justify-center text-white"
          ...
          <div 
            v-if="showLyrics" 
            class="w-[75vw] mb-6 overflow-y-auto max-h-[40vh]"
          >
            <h3 class="text-xl font-bold mb-2 text-center">{{ audioStore.currentTrack.title }}</h3>
            <p class="text-gray-600 text-sm mb-4 text-center">{{ audioStore.currentTrack.artist }}</p>
            <pre class="whitespace-pre-wrap font-sans text-gray-800 leading-relaxed text-sm mb-4">{{ audioStore.currentTrack.lyrics || 'No lyrics available' }}</pre>
            
            <!-- Contributors Badges (Mobile) -->
            <div v-if="getContributors(audioStore.currentTrack.contributors).length > 0" class="flex flex-wrap gap-2 justify-center mt-4">
              <span 
          ...
          <div v-if="!showLyrics">
            ...
            <p class="text-lg text-gray-600 mb-6 text-center">{{ audioStore.currentTrack.artist || 'Unknown Artist' }}</p>
          </div>
          ...
          <div class="flex items-center space-x-6">
            <button class="p-3 bg-gray-300 text-gray-700 rounded-full hover:bg-red-600 hover:text-white transition">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
              </svg>
            </button>
            ...
            >
              <svg v-if="audioStore.isPlaying" xmlns="http://www.w3.org/2000/svg" class="h-10 w-10" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 9v6m4-6v6" />
              </svg>
        ...
        <div class="hidden md:flex md:flex-row w-full gap-8">
          ...
          <div class="flex-shrink-0 flex flex-col items-center md:w-1/2">
            <div 
              class="w-96 h-96 bg-gray-200 rounded-lg shadow-xl mb-6 overflow-hidden"
            >
              <img 
                v-if="audioStore.currentTrack.cover_url" 
                :src="audioStore.currentTrack.cover_url" 
                ...
                class="w-full h-full object-cover"
              />
              <div v-else class="w-full h-full bg-gradient-to-br from-red-600 to-gray-800 flex items-center justify-center text-white text-4xl">
                ♪
              </div>
            </div>
            
            <h2 class="text-3xl font-bold mb-2 text-center">{{ audioStore.currentTrack.title }}</h2>
            <p class="text-xl text-gray-600 mb-6 text-center">{{ audioStore.currentTrack.artist || 'Unknown Artist' }}</p>
            ...
            <div class="flex items-center space-x-8">
              <button class="p-4 bg-gray-300 text-gray-700 rounded-full hover:bg-red-600 hover:text-white transition">
          ...
          <div class="md:w-1/2 flex flex-col bg-gray-50 rounded-lg p-6 overflow-y-auto">
            ...
            <div class="flex-1 overflow-y-auto mb-4">
              <pre class="whitespace-pre-wrap font-sans text-gray-800 leading-relaxed">{{ audioStore.currentTrack.lyrics || 'No lyrics available' }}</pre>
            </div>
            
            <!-- Contributors Badges (Desktop) -->
            <div v-if="getContributors(audioStore.currentTrack.contributors).length > 0" class="flex flex-wrap gap-2 mt-4 pt-4 border-t border-gray-200">
              <span 
                v-for="contributor in getContributors(audioStore.currentTrack.contributors)" 
                :key="contributor"
...
const getContributors = (contributorsString?: string): string[] => {
  ...
  return contributorsString.split(',').map(c => c.trim()).filter(c => c.length > 0)
}
</script>
